...
没有使用真实界面截图、生成漫画样例或性能数字。两张概览图均明确标为“流程示意 · 非界面截图”。徽章是静态版本要求 / 开发入口 / MVP 阶段说明，不模拟实时 CI、许可证或供应商可用状态。
...
| [overview.svg](overview.svg) / [overview.png](overview.png) | 桌面流程图，920 × 620 逻辑画布，PNG 为 2 倍分辨率 |
| [overview-mobile.svg](overview-mobile.svg) / [overview-mobile.png](overview-mobile.png) | 窄屏纵向图，430 × 904 逻辑画布，PNG 为 2 倍分辨率 |

README 用 `<picture>` 在窄屏选择纵向 PNG；不支持该选择的阅读器回退到桌面 PNG。PNG 用于固定中文字体与版面，SVG 保留为可编辑来源。核心流程在 README 正文和图片替代文本中均有说明，不依赖图片传递唯一信息。
