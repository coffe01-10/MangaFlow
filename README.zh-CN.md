...
[宣传视频](#宣传视频) · [工作方式](#工作方式) · [Windows 桌面版](#windows-桌面版) · [快速开始](#快速开始) · [开发与验证](#开发与验证) · [文档导航](#文档导航)

> [!IMPORTANT]
> 当前 Windows 版本为 `v1.0.0-rc2` 发布候选。主流程已有实现，但安装器尚未进行代码签名，真实供应商、多显示器/DPI 和长会话仍有未完成验收，不应视为稳定生产版本。具体影响与修复优先级见[主分支路线图](docs/roadmap.md)。
...
| 编排重复的创作步骤 | 可编辑、发布和运行的 DAG 工作流；全屏与专注模式、可收起面板、画布导航、持久化任务和人工确认节点 |
| 查看模型用量与成本 | 按真实派发记录供应商/模型、输入/输出/缓存 Token、图片数、版本化价格估算、CSV 导出与独立账单对账 |

这些是已有功能入口，不代表所有异常路径都已验收。尤其不要仅凭“任务取消成功”判断已停止外部调用，也不要仅凭质检汇总状态判定成品可交付；当前缺陷见[路线图](docs/roadmap.md)。

## Windows 桌面版

[下载 MangaFlow Desktop `v1.0.0-rc2` Windows x64 版](https://github.com/coffe01-10/MangaFlow/releases/tag/v1.0.0-rc2)。Release 提供按用户安装的 NSIS 安装器与 SHA-256 校验文件；客户端基于原生 WPF，目标运行时为 .NET 10 Windows Desktop Runtime，不依赖 WebView。

安装器支持全新安装与原位覆盖升级；卸载应用不会删除 `%LOCALAPPDATA%\MangaFlow\Native` 下的项目数据。当前候选版尚未进行 Authenticode 签名，Windows 可能显示“未知发布者”，运行前可用 Release 附带的校验文件核对安装器。

`rc2` 的流程编排使用工作台统一的纸色、墨色和朱红主题，默认以 100% 显示。节点库、属性面板、版本状态、运行面板和小地图统一由“视图”菜单控制：

- `F11` 切换窗口全屏；`Esc` 恢复进入前的窗口状态。
- `Ctrl+Shift+Enter` 切换专注模式；退出后恢复此前的面板布局。
- `Shift+滚轮` 横向移动，`Ctrl+滚轮` 缩放，普通滚轮纵向移动。
- 空格＋拖动或中键拖动用于平移画布，“查看全图”适配整个流程范围。

下面的源码启动步骤继续用于开发贡献和运行 Web 工作台。
...
计费窗口不会根据界面中的文本自行数 Token，也不会截取实时 Token 流。后端在每次可能产生费用的供应商派发前先提交一条待完成审计记录；调用结束后，适配器保存供应商最终响应里的 `usage` 或 `usage_metadata`，CLI 通道则读取最终结果事件。OpenAI 兼容、Gemini/Vertex、缓存输入、推理 Token 和图片数量等字段会归一到同一本账，同时保留原始响应供诊断。

供应商未返回用量时保持“未知”，不会写成 `0`。供应商已经受理并可能计费的失败请求仍可记录用量；每次真实重试或切换密钥都会单独记一笔。看板按调用发生时生效的价格版本计算估算金额，供应商账单对账单独展示，不同币种也不会相加。桌面页在进入页面、切换筛选条件和刷新时读取数据库快照，因此最终 Token 通常在调用完成落账后出现，而不是逐 Token 跳动。数据模型与边界见[用量账本契约](docs/v02-usage-ledger-contract.md)。
