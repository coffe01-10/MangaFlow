...
def test_deleted_project_cancellation_is_committed(monkeypatch):
    with TemporaryDirectory() as directory:
        engine = create_engine(
            f"sqlite:///{Path(directory) / 'deleted-project.db'}",
            connect_args={"check_same_thread": False},
        )
        testing_session = sessionmaker(
            bind=engine, autoflush=False, expire_on_commit=False
        ...
        Base.metadata.create_all(engine)
        with testing_session() as db:
            project = Project(name="已删除项目", deleted_at=datetime.now(UTC))
            db.add(project)
            db.flush()
            job = GenerationJob(
                project_id=project.id,
                target_type="CHAPTER",
                target_id="deleted-target",
                job_type="SOURCE_PARSE",
                status=JobStatus.QUEUED,
            )
            db.add(job)
...
def test_asset_generation_accepts_generated_outfit_reference(db_session):
    project = Project(name="生成服装参考")
    db_session.add(project)
    db_session.flush()
    character = Character(project_id=project.id, primary_name="主角")
    asset = Asset(
        project_id=project.id,
        kind="page_candidate",
        original_name="generated-outfit.png",
        storage_key="generated/generated-outfit.png",
        mime_type="image/png",
        byte_size=10,
        sha256="g" * 64,
        source="AI_GENERATED",
    ...
    references = worker_tasks._outfit_reference_assets(db_session, outfit, project.id)
