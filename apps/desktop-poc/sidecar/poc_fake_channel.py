"""Fake model channel for the V02-53B desktop shell PoC (zero provider calls).

This module is PoC-owned acceptance tooling, not product code. Following the
...
class FakePocChannelAdapter:
    ...
    def _image_response(self) -> ModelResponse:
        ...
        return ModelResponse(
            model_id="poc-fake-image",
            request_id=f"poc-fake-request-{self.call_count}",
...
def _seed_catalog() -> None:
    ...
    with SessionLocal() as db:
        existing = db.query(ProviderProfile).filter_by(name="PoC Fake Channel").one_or_none()
        if existing is not None:
            return
        profile = ProviderProfile(
            name="PoC Fake Channel",
            category="LOCAL",
            description="V02-53B disposable PoC fake channel; never called over network",
            enabled=True,
        )
        db.add(profile)
        db.flush()
        connection = ProviderConnection(
            ...
            enabled=True,
            message="PoC fake channel; base URL is intentionally unreachable",
        )
        db.add(connection)
        db.flush()
        ...
        db.add(
            ProviderKey(
                ...
                encrypted_secret=encrypt_secret(get_settings(), "poc-fake-channel-key"),
                key_hint="••••",
            )
        )
        for model_type, display, legacy_alias in (
            ("IMAGE", "PoC 假图片模型", IMAGE_LEGACY_ALIAS),
            ("TEXT", "PoC 假文本模型", "poc.fake.text"),
        ):
            db.add(
                AIModel(
                    connection_id=connection.id,
                    provider_model_id=f"poc-fake-{model_type.lower()}",
...
def install() -> None:
    ...
    _seed_catalog()
    worker_tasks._adapter = lambda _alias: FakePocChannelAdapter()
