...
const POC_ROOT = new URL("..", import.meta.url).pathname;
const REPO_ROOT = new URL("../../..", import.meta.url).pathname;
const FRONTEND = join(POC_ROOT, "dist/frontend");
const HELPER = join(POC_ROOT, "sidecar/mangaflow_poc_helper.py");
const PYTHON = process.env.MANGAFLOW_POC_PYTHON ?? "python3";
...
const user_data = await (async () => {
  ...
  return mkdtemp(join(tmpdir(), "mangaflow-poc-d5-"));
})();
const runtime = join(user_data, `runtime/mangaflow-poc-${token}`);
...
const helper = spawn(PYTHON, [HELPER, "app", "--api-root", join(REPO_ROOT, "apps/api"),
  ...
  env: { ...process.env, MANGAFLOW_POC_TOKEN: token, MANGAFLOW_POC_JOURNAL: journal,
