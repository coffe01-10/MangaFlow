#!/usr/bin/env bash
# V02-53B D2: package the Python sidecar with PyInstaller (Linux form).
#
...
if [ ! -x "$VENV/bin/pyinstaller" ]; then
  echo "run scripts/run-sidecar-e2e.sh once to create .venv-poc (needs pyinstaller)" >&2
  exit 1
fi

cd "$REPO_ROOT"
"$VENV/bin/pyinstaller" --noconfirm --clean --onedir --name mangaflow-poc-sidecar \
  --distpath "$POC_ROOT/dist/sidecar" --workpath /tmp/mangaflow-poc-pyi --specpath /tmp/mangaflow-poc-pyi \
  --paths apps/api --paths apps/desktop-poc/sidecar \
  ...
  --hidden-import app.api.router --hidden-import poc_fake_channel \
  --collect-submodules app \
  apps/desktop-poc/sidecar/mangaflow_poc_helper.py
...
LAYOUT="$POC_ROOT/dist/sidecar/mangaflow-poc-sidecar"
...
echo "smoke: $LAYOUT/mangaflow-poc-sidecar app --api-root $LAYOUT/_internal --user-data <dir> --fake-channel"
