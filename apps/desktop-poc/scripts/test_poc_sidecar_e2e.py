"""V02-53B PoC end-to-end: real API sidecar + fake model channel.
...
Run via scripts/run-sidecar-e2e.sh (needs .venv-poc with apps/api deps).
"""
...
HELPER = REPO_ROOT / "apps/desktop-poc/sidecar/mangaflow_poc_helper.py"
...
class PocShell:
    ...
    def __init__(self, user_data: Path) -> None:
        ...
        self.runtime = user_data / "runtime" / f"mangaflow-poc-{self.token}"
        self.runtime.mkdir(parents=True)
        self.journal = self.runtime / "owner.json"
        self.stderr_log = (self.runtime / "helper.stderr.log").open("wb")
        env = dict(
            os.environ,
            MANGAFLOW_POC_TOKEN=self.token,
            MANGAFLOW_POC_JOURNAL=str(self.journal),
...
@pytest.fixture()
def poc(tmp_path: Path):
    user_data = tmp_path / "user-data"
    (user_data / "data").mkdir(parents=True)
    shell = PocShell(user_data)
    try:
...
def _client(shell: PocShell) -> httpx.Client:
...
def test_sidecar_boot_and_fake_generate_candidate_loop(poc):
    shell, user_data, record = poc
    client = _client(shell)

    project = client.post(
        "/projects",
        json={"name": "V02-53B 桌面壳 PoC", "workflow_mode": "AUTO", "default_concurrency": 2},
    )
    ...
    assert not (REPO_ROOT / "storage" / "mangaflow-poc").exists()
