        ...
        <section className="workspace-canvas">
          ...
          {section === "library" && (
            <>
              ...
              <nav className="library-scope-tabs" aria-label="按章节筛选生成批次"><button type="button" aria-pressed={!libraryChapter} className={!libraryChapter ? "active" : ""} onClick={() => { setLibraryChapter(""); setLibraryCursor(""); setLibraryHistory([]); }}><span>ALL</span><strong>全部素材</strong></button>{chapters.data?.map((chapter) => <button type="button" aria-pressed={libraryChapter === chapter.id} className={libraryChapter === chapter.id ? "active" : ""} key={chapter.id} onClick={() => { setLibraryChapter(chapter.id); setLibraryCursor(""); setLibraryHistory([]); }}><span>{String(chapter.ordinal).padStart(2, "0")}</span><strong>{chapter.title}</strong></button>)}</nav>
