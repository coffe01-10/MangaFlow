      ...
      <section className={`${styles.body} ${libraryOpen ? "" : styles.libraryClosed} ${inspectorOpen ? "" : styles.inspectorClosed}`}>
        ...
        <aside className={styles.inspector}>
          ...
          {selected ? <div className={styles.inspectorForm}>
            ...
            {selected.data.graphNode.type === "generator.page" ? <><label>模型由每次生成选择<input value="显式选择供应商模型 / 自动路由" disabled /></label><label>建议清晰度<select value={selected.data.graphNode.config.resolution ?? "1K"} onChange={(event) => updateSelected({}, { resolution: event.target.value as Resolution })}><option>1K</option><option>2K</option><option>4K</option></select></label></> : null}
