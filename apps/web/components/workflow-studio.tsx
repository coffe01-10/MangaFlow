  ...
  return (
    <main className={styles.studio}>
      ...
      <section className={`${styles.body} ${libraryOpen ? "" : styles.libraryClosed} ${inspectorOpen ? "" : styles.inspectorClosed}`}>
        ...
        <aside className={styles.inspector}>
          ...
          {selected ? <div className={styles.inspectorForm}>
            ...
            {selected.data.graphNode.type === "generator.page" ? <><label>模型由每次生成选择<input value="Nano Banana 2 / Pro 平级" disabled /></label><label>建议清晰度<select value={selected.data.graphNode.config.resolution ?? "1K"} onChange={(event) => updateSelected({}, { resolution: event.target.value as Resolution })}><option>1K</option><option>2K</option><option>4K</option></select></label></> : null}
            {selected.data.graphNode.config.model_alias === "text.fast" ? <><label>文本模型<input value="Gemini 3.5 Flash" disabled /></label><label>温度<input type="number" min="0" max="2" step="0.1" value={selected.data.graphNode.config.temperature} onChange={(event) => updateSelected({}, { temperature: Number(event.target.value) })} /></label></> : null}
      ...
      <footer className={styles.runner}>
        ...
        {displayedRun?.node_runs.filter((run) => run.status === "WAITING_APPROVAL").map((run) => <div className={styles.approval} key={run.id}><strong>{run.node_type === "generator.page" ? "单页生成等待选择模型" : "采用候选后继续"}</strong>{run.node_type === "generator.page" ? <><select value={drawModel} onChange={(event) => setDrawModel(event.target.value as ImageModelAlias | "")}><option value="">选择平级模型</option><option value="image.nano_banana_2">Nano Banana 2</option><option value="image.nano_banana_pro">Nano Banana Pro</option></select><select value={drawResolution} onChange={(event) => setDrawResolution(event.target.value as Resolution)}><option>1K</option><option>2K</option><option>4K</option></select></> : <Link href={`/projects/${projectId}/generate`}>前往采用</Link>}<button disabled={approveNode.isPending || (run.node_type === "generator.page" && !drawModel)} onClick={() => approveNode.mutate(run)}>确认继续</button></div>)}
      </footer>
    </main>
  );
}
