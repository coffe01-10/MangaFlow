      ...
      <section className={`${styles.body} ${libraryOpen ? "" : styles.libraryClosed} ${inspectorOpen ? "" : styles.inspectorClosed}`}>
        ...
        <aside className={styles.inspector}>
          ...
          {selected ? <div className={styles.inspectorForm}>
            ...
            {selected.data.graphNode.config.model_alias ? <><label>文本模型<select value={selected.data.graphNode.config.model_alias} onChange={(event) => updateSelected({}, { model_alias: event.target.value })}>{selectedTextModels.map((model) => <option key={model.catalog_id} value={model.logical_alias}>{model.provider} · {model.display_name}</option>)}</select></label><label>温度<input type="number" min="0" max="2" step="0.1" value={selected.data.graphNode.config.temperature} onChange={(event) => {
              // 半输入（"1e"）解析为 NaN 会被防抖自动保存当成 null 提交；无效输入保持旧值。
              const parsed = Number(event.target.value);
              if (event.target.value.trim() !== "" && Number.isFinite(parsed)) updateSelected({}, { temperature: Math.min(2, Math.max(0, parsed)) });
            }} /></label></> : null}
            <label>超时（秒）<input type="number" min="30" max="3600" value={selected.data.graphNode.config.timeout_seconds} onChange={(event) => {
              const parsed = Number(event.target.value);
              if (event.target.value.trim() !== "" && Number.isFinite(parsed)) updateSelected({}, { timeout_seconds: Math.min(3600, Math.max(30, Math.round(parsed))) });
            }} /></label>
            <label>重试次数<input type="number" min="1" max="10" value={selected.data.graphNode.config.max_attempts} onChange={(event) => {
              const parsed = Number(event.target.value);
              if (event.target.value.trim() !== "" && Number.isFinite(parsed)) updateSelected({}, { max_attempts: Math.min(10, Math.max(1, Math.round(parsed))) });
            }} /></label>
            ...
            {selected.data.graphNode.type === "control.condition" ? <><label>JSON 路径<input value={String(selected.data.graphNode.config.condition.path ?? "$")} onChange={(event) => updateSelected({}, { condition: { ...selected.data.graphNode.config.condition, path: event.target.value } })} /></label><label>比较符<select value={String(selected.data.graphNode.config.condition.operator ?? "exists")} onChange={(event) => updateSelected({}, { condition: { ...selected.data.graphNode.config.condition, operator: event.target.value } })}><option value="exists">存在</option><option value="eq">等于</option><option value="ne">不等于</option><option value="contains">包含</option><option value="gt">大于</option><option value="gte">大于等于</option><option value="lt">小于</option><option value="lte">小于等于</option></select></label><label>比较值<input value={selected.data.graphNode.config.condition.value === undefined || selected.data.graphNode.config.condition.value === null ? "" : String(selected.data.graphNode.config.condition.value)} onChange={(event) => updateSelected({}, { condition: { ...selected.data.graphNode.config.condition, value: event.target.value } })} placeholder="除“存在”外必须填写" /></label></> : null}
