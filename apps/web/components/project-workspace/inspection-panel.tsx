...
export function InspectionPanel({
  ...
  return <section className="inspection-panel">
    ...
    {!latestInspections.length ? <div className="inspection-wait" role="status" aria-live="polite"><LoaderCircle className={reviewJob && !isTerminalTaskStatus(reviewJob.status) ? "spin" : ""} size={18} /><span>{reviewJob ? `检查任务 ${jobStatusLabels[reviewJob.status] ?? reviewJob.status} · ${reviewJob.progress}%` : "正在读取检查结果"}</span></div> : <div className="inspection-results">{latestInspections.map((inspection) => {
      ...
      return <article className={passed ? "passed" : "failed"} key={inspection.id}>
        <div><span>{inspectionLabels[inspection.category] ?? inspection.category}</span><strong>{inspectionOutcomeLabels[inspection.outcome] ?? inspection.outcome}</strong><em>{inspection.score === null ? "—" : `${Math.round(inspection.score * 100)}%`}</em></div>
    ...
    {(inspectCandidate.isError || repairCandidate.isError || upscaleCandidate.isError) && <p className="form-error" role="alert"><CircleAlert size={14} />{(inspectCandidate.error ?? repairCandidate.error ?? upscaleCandidate.error)?.message}</p>}
  </section>;
}
