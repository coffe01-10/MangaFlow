          ...
          <div className="asset-grid">{grouped.map((asset, index) => {
            ...
            return <article className={selected ? "asset-card selected" : "asset-card"} key={asset.id}>
              ...
              <div><AssetNameEditor asset={asset} pending={renameAsset.isPending && renameAsset.variables?.assetId === asset.id} error={renameAsset.isError && renameAsset.variables?.assetId === asset.id ? renameAsset.error : null} onSave={(displayName) => renameAsset.mutate({ assetId: asset.id, displayName })} /><p>{label} · {formatBytes(asset.byte_size)}</p><span className="tiny-status"><Check size={11} />{assetStatusLabels[asset.status] ?? asset.status}</span>
                ...
                {/* 重分类会解除既有角色/服装/风格/场景绑定且不可撤销（#165）：select 提交 pending 期间禁用；变更先回退显示并确认，取消即还原。 */}
                <div className="asset-actions"><select aria-label="修改素材用途" value={asset.kind} disabled={reclassifyAsset.isPending} onChange={(event) => { const nextKind = event.target.value as AssetPurpose; if (nextKind === asset.kind) return; const currentLabel = kinds.find(([value]) => value === asset.kind)?.[1] ?? asset.kind; const nextLabel = kinds.find(([value]) => value === nextKind)?.[1] ?? nextKind; event.target.value = asset.kind; if (window.confirm(`将把「${assetName(asset)}」的用途从「${currentLabel}」改为「${nextLabel}」，可能解除已有绑定。确定继续吗？`)) reclassifyAsset.mutate({ assetId: asset.id, kind: nextKind }); }}>{kinds.map(([value, option]) => <option key={value} value={value}>{option}</option>)}</select><button title="删除素材" disabled={deleteAsset.isPending} onClick={() => { if (window.confirm("删除该素材及其候选记录，并解除已有绑定？")) deleteAsset.mutate(asset.id); }}><Trash2 size={13} /></button></div>
              </div>
            </article>;
          })}</div>
