...
流程编排使用工作台的共享主题色，默认以 100% 显示节点。顶部「视图」菜单统一控制节点库、属性面板、版本状态、运行面板和小地图。选中节点会打开属性面板；专注模式下保持画布优先，可从「视图」手动打开属性。

- `F11`：切换真正的窗口全屏；`Esc` 退出并恢复原窗口大小。
- `Ctrl+Shift+Enter`：切换专注模式，收起导航和辅助面板；再次切换或按 `Esc` 恢复进入前的面板状态。
- `Shift+滚轮`：横向移动；普通滚轮纵向移动；`Ctrl+滚轮` 缩放。
- 空格＋拖动或中键拖动：平移画布；「查看全图」适配流程范围。

全屏与专注模式可以组合使用。`Esc` 优先取消正在拖拽的连线，然后依次退出全屏、专注模式。保存失败等通知在专注模式下仍会显示。
