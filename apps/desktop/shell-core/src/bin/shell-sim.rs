...
use mangaflow_desktop_shell_core::handshake::{self, spawn_helper, HelperConfig};
...
fn main() {
    ...
    if helper.is_empty() || user_data.is_empty() {
        eprintln!("MANGAFLOW_DESKTOP_HELPER and MANGAFLOW_DESKTOP_USER_DATA are required");
        std::process::exit(2);
    }
