...
def test_support_files_land_inside_internal():
    source = _SCRIPT.read_text(encoding="utf-8")
    internal = source.index("_internal")
    assert source.index("alembic.ini") < internal or "alembic.ini" in source[
        internal:
    ], "alembic.ini must be copied beside the frozen app"
    assert 'mkdir -p "$LAYOUT/_internal"' in source, (
        "the _internal directory must be created explicitly"
    )
    assert "cp -r" in source and "migrations" in source, (
        "migrations/ must be copied recursively into the bundle"
    )
