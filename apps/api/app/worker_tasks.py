...
class JobLeaseLostError(RuntimeError):
    """Stop an old worker after another worker reclaimed the job."""
...
def _run_asset_generate(db, job: GenerationJob) -> None:
    ...
    if batch.target_type == "CHARACTER":
        ...
        outfit_references = _outfit_reference_assets(db, outfit, batch.project_id)
...
def _outfit_reference_assets(db, outfit: Outfit, project_id: str) -> list[Asset]:
        ...
        db.scalars(
            select(Asset).where(
                ...
                or_(
                    ...
                    Asset.source.in_({"AI_GENERATED", "VERTEX_GENERATED"}),
                ),
            )
        )
    )
