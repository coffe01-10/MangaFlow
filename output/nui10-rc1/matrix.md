...
| P2-4b G2 三档 net10 | BLOCKED（首轮中断，待空闲整轮重跑） | 首轮 tier100（scripts/nui10_g2_tier.ps1，run-dir nui10-g2-tier）执行至 `--no-seed` 重启成功（20:53 栈就绪、shell 日志 ready_verified→healthy）后测量未落盘：MangaFlow.Native 进程在采样开始前消失（Windows 事件日志无 .NET/WER 崩溃记录、shell 日志无 stopped 事件、native-host 存活），同时用户回到实机操作（ZCode 前台弹窗）；按实机独占规则中止，已 stop 清栈（issues none）。CSV 未产出，100/300/500/600 全部待整轮重跑；net8 对照基线 output/nui9-rc/p5-g2/ 不变 |
...
- 2026-09-20：WPS 退出前台后重启 nui10-live4（--no-seed，wpf 23764）复验连线拖拽：**PASS**（V6→V7 + 新边 n1:source→agent-parse:source 落库，画布分叉线/撤销激活/已保存三重画布证据）。排查中定位前轮「合成点击失效」真根因=测试脚本坐标系误读（PS 为 system-DPI-aware，截图像素=物理像素，无需 ÷1.25；失效点击实际落在空白区），应用输入链路无缺陷——已回写 P4-3b 根因更正。随后 G2 tier100 首轮（新驱动 scripts/nui10_g2_tier.ps1）在 --no-seed 重启后测量前应用进程消失（无崩溃记录、无 stopped 事件），且用户回到实机（ZCode 前台），按实机独占规则中止并清栈；P2-4b 记 BLOCKED 待整轮重跑。工具经验：输入探针一律以截图像素=物理像素直接落点、WindowFromPoint 守卫保留、多行 python -c 在 cmd 下静默空转（必须写临时 .py 文件）。
