...
| P5-4 收工复核 | 三件套 2/3 绿，--render 待安静窗口重跑 | ①npm run check：**1664 passed + 47 passed / 56 files 615 tests / build ✓**（p5-npm-check-final.log；+1 为种子回归测试，已解释）②dotnet build -c Release **0 error** ✓ ③进程/端口复核 ✓（G2 四档收尾后 MangaFlow.Native/native-host 无残留、3000/8000 无监听）④`--render` 终跑：跑于 22:4x，**失败=`零位移手势不得进入撤销栈`**（NativeStoryboardEditChecks.PanelDragGestureChecks:512）——离屏手势检查期间前台为用户 Chrome（活跃浏览），物理鼠标移动被离屏窗口的输入泵观测为非零位移，判定受实机干扰无效（log `render-net10-final.log` 保留为证据）；同一二进制在 P2-3 安静窗曾 56 项 PASS。**待安静窗口重跑后本格才能勾** |
