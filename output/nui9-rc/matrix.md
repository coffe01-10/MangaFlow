...
| 格 | 状态 | 证据 |
| --- | --- | --- |
| P4-1 七格窗口模态 M2/M3/M5~M9（Esc/焦点返还/双支线） | 部分通过（2/7 实机：M4 Lightbox、M5 JobDetails） | M4 Lightbox：模态窗口 [批次候选 1] → Esc 关闭且焦点返还触发按钮。M5 JobDetailsWindow：任务中心点 [调用与成本] → 窗口 [调用与成本 · PAGE_GENERATION] 打开 → Esc 关闭 → 焦点回 [调用与成本] 触发钮。余下五格（M2 ConfirmDialog 重试任务、M3 InputDialog 重命名参考页、M6 LocalEditWindow、M7 SceneEditor、M8 PanelEditDialog、M9 LayoutRebuildDialog）待续跑。注意 M2 重试确认支线属付费 AUTH-REQ，仅跑取消支线 |
