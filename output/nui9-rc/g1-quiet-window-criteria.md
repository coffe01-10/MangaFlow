# P5-2 G1「安静测量窗口」口径（定稿 2026-09-20，NUI-10 P1-1）

> 定稿：lead 于 NUI-10 goal 轮确认本口径（安静窗口四条件、样本全保留 + DISTURBED 分列、
> 首帧 = 壳窗口首帧可见不含 provisioning），并指定先在 **net8** 上跑一次 G1 N=20 作为
> 迁移前参照（不设达标线，只建对照；否则 net10 数字无基线可比）。以下正文即定稿内容，
> 执行记录见 `output/nui10-rc1/matrix.md` P1-2 格与 `output/nui10-rc1/p1-g1/`。
