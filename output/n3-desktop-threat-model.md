...
- **R13（周末窗续，ed99ad5：#533）**：复核 6c62051（_read_context 畸形 token 拒绝表：尾随换行/空格/大写 hex/超长/非 hex/空——token 是 runtime 目录的文件名锚，宽松匹配会让伪造 env 指向外来 runtime 目录；六形态全拒正确）与 400b2dd（wave-1 委托票的 native check 接线 + harness 加固）。认证 HEAD：shell-core **150/150**、sidecar 全家 **82/82**。
...
## 9. 周末窗口台账（20260912-wknd，窗口至 09-14 09:00；基线 f43ac71）

**B — Issue 账（周末窗新立）：**
- #507 [P3] mid-session node 死亡后公告中继继续转发到未重验证的 node_port——回收绑定者收到 WebView 流量（双端 file:line 证据；修复面：exit watch 关闭公告中继或 accept 循环活性检查）
- #508 [P4] e2e harness Windows stop() 欠杀其接受的孙进程 announcer——孤儿 helper + 永久阻塞的 reader 线程（POSIX 干净；修复面：Job Object 或按 READY pid 击杀）

**C — 防守 PR 账（周末窗）：** 待立（候选：#507 的 accept 活性检查、#508 的 Job Object harness）。
