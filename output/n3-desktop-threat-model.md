...
基线：origin/master `fe22969`（20260911 夜；历次基线 961bcec/929a216/de1b999 的产出均已合并，见 §6/§8）（含 2026-09-07 夜的 #271/#273/#274/#277 修复与 #259/#270，2026-09-08/09 夜并行轮的 #278/#279/#287/#289/#291、fake-channel 门控 0dfaac3、native 加固 6aee185 等）。
...
- #342 [P3][native] 工作流自动保存 debounce 未在切换/导入时解除——陈旧定时器把 A 的图 PATCH 进 B（:1052-1066/:252-260）
- #343 [P3] env/api-root 回归套件无任何 runner 收集 + 纯函数测试未钉 wiring 顺序（run-sidecar-e2e.sh:27-31）→ **已修 PR #352 + #354 扩展**
- #344 [P2][native] #340 伴随形状：condition 嵌套值与 undo 快照的共享身份（修复时需深拷贝覆盖）
- #345 [P3] README 桌面章节与实现矛盾（plan-B "未被验证" vs 已交付+17 e2e；31 个迁移；脚本地图缺 9 个）
- #346 [P3] D5 就绪超时孤儿 helper + GO 探活无超时 → **已修 PR #353 + #357 完整收口**
- #347 [P3] assemble-web-resources.py 非原子重建——中途失败留下截断的安装器资源树
- #348 [P4] measure-native-startup.ps1 超时只杀根进程——sidecar 树孤儿持端口；Ctrl+C 全漏
- #349 [P4] e2e fixture finally 断言掩蔽测试体失败的真实原因 → **已修 PR #354**
- #350 [P4] dist/ 构建产物无跨进程锁——并发 build/e2e/D5 互相破坏
- #351 [P4] 壳工具菜单在窗口关闭后静默失效（无重建路径）

**C — 防守钉 PR 账（本 Goal 内 4 个）：**
- #352 runner 收集 env/api-root 套件（合并）
- #353 D5 就绪超时杀进程组 + 探活 2s 上限（合并；R2 复核发现 headline 路径绕过 fail → #357 完整收口）
- #354 e2e finally 掩蔽修复（fixture + plan-B + 三个新增测试全量覆盖）
- #357 栅栏击穿必须使 CI 失败 + 就绪超时杀进程组 + spawn 期 exit 承诺（待合并）

**E — 互审轮次（本 Goal 4 轮）：**
- R1：三路并行审计（native 深审 / shell-sidecar-src-tauri 增量 / scripts+docs 一致性）——产出上列全部 Issue 候选
