...
## 34. 夜班 20260913（新分支 night/n2-platform-burn-20260913，基线 5d8730e；账本自 wknd 分支续）

- **日班 scope 落地交叉清单**：#602（journal 终态竞态：per-writer pending 名 + 终态守卫
  best-effort 文档化）、#588（D5 拆除 kill 走 Windows 可用 helper）、大小写变体 env scrub、
  我方 #622（static-export 构建 scrub）已并；新增 test_sidecar_journal.py（230 行）与
  test_verify_static_origin.py（205 行）。
- **PR #674（夜班第一枚，待 lead）**：上述两文件**未进 runner 收集列表**——按 runner 自述
  #343 规则即"未列 = 未测"，435 行契约 canonical 路径不执行。列入列表。
- **串行证据（含一次真实门触发）**：新列表首跑 1 failed——**溯源门按设计响亮**（白天
  apps/web r7 使 bundle 过期，补救文案 + #608 日志留身份全数生效）；重建后 **146 passed
  in 74.23s**（exit=0）。
- **第 22 轮评审（1 子代理，文件/行级，APPROVE）**：16 passed 无自跳过；列表追加无固定
  端口/路径冲突（journal 纯 tmp_path、verify 为纯静态字符串断言不 spawn）；无重复收集；
...
- **§35 补遗三**：#687/#688/#689（他组 glm/* 堆叠链：ledger-23 → plan-B 运行时 CSP 钉 →
  ledger-24）三条 CONFLICTING 已按 lead 指令全部 rebase 到 9fc7371 链（687→ledger-23；
...
- **§35 补遗二十三（窗口收尾认证，tip 898 合并后）**：真 runner **180 passed + 2 skipped**
