...
> **2026-08-27 进展：** 主分支可靠性收口的 P0（Worker 租约恢复、启动迁移校验）、任务幂等插入竞态与前端活动任务轮询均已完成；提交 `d965007`～`390ebdc` 及其后继修复见 [`roadmap.md`](roadmap.md)。剩余缺口集中在浏览器验收基线（首页请求预算与 WCAG 对比度）和完整门禁脚本。
>
...
| H. 主分支可靠性收口 | 进行中 | P0 全部完成（租约恢复/回收、启动迁移校验、幂等与轮询竞态）；剩余浏览器请求预算、WCAG 对比度、`check:full` 完整门禁与序号竞态确认 |
