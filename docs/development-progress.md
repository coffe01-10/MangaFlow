...
- **U1–U10 矩阵（§13）证据**：
  ...
  - **U6 RUN**：断言 `prefers-reduced-motion` 全表仅一段；`.01ms` 冻结 + iteration 1（spinner 可停）；带 translate 的 hover 位移取消用 `transform: none !important` 覆盖全部 20 个位移源（含全局 tactile 块的 `button:hover:not(:disabled)` / `a.button:hover`——该块位于合并块之后，若无 `!important` 会按优先级反超），并有解析器守卫测试逐个核对全表 hover+translate 选择器 ⊆ 覆盖清单；`rotate(-4deg)` 全表消失。
