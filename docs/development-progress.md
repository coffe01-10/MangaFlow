...
| H. 主分支可靠性收口 | 进行中 | P0 与 P1 全部完成（租约、迁移校验、幂等/轮询、浏览器基线 4/4 通过、`check:full` 完整门禁）；剩余序号竞态确认、分支清理与 P2 运营项 |
...
- [x] 2026-08-27 浏览器门禁恢复：Playwright/Axe E2E 4/4 通过；首页首屏 API 请求降为 dashboard + vertex status 共 2 个（预算 ≤3）；Dashboard 新增 `ai_overview` 摘要，全部被测页面 Axe WCAG 2 AA serious/critical 为零。
