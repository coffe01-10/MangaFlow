...
## v1.0.0-rc2 已发布（2026-09-21，发布提交 `e0b00412`）

- GitHub Release `v1.0.0-rc2` 已发布 Windows x64 NSIS 安装器、SHA-256 校验文件和发行说明；安装器为 28,972,821 bytes，SHA-256 `7B260A79AAAA2AECC4D2E7FAD6844A1CE74A31C7DAF22948DF35E92D60D9E16E`。
- 全新安装、原位覆盖升级、DisplayVersion、40 秒启动探测、sidecar 健康握手/停止和卸载清理通过。启动新增的数据库、日志和 runtime 身份文件符合预期，既有用户数据标记在安装/升级/卸载过程中保留。
- 当前边界：安装器未做 Authenticode 签名；多显示器/高 DPI、真实 Provider/Worker 付费闭环与长会话画布帧时间未在本轮运行。

## 原生流程编排展示与布局收口（2026-09-21，`91c4111f`）

- 新增 F11 窗口全屏、Ctrl+Shift+Enter 专注模式和 Esc 分级退出，恢复进入前的窗口大小与辅助面板状态；保存失败通知在专注模式下继续可见。
- “视图”菜单统一节点库、属性面板、版本与保存状态、运行面板、小地图；选中节点自动打开属性面板，项目内入口和运行按钮去重。
- 画布支持 Shift+滚轮横向移动、普通滚轮纵向移动、Ctrl+滚轮缩放、空格/中键拖动画布；滚动范围随缩放和节点范围更新。
- 使用桌面共享纸色/墨色/朱红主题，默认 100% 缩放，增大节点标题与端口文字，并改善 650–1440 DIP 下的工具栏换行和抽屉行为。
- 验证：原生 Release 工作流/侧栏回归通过；`npm run check` 全绿（Python 1664 passed / 47 skipped、Vitest 615 passed、Next.js 生产构建通过）。证据见 `output/workflow-ux-20260921/verification.md`。
...
- **修复轮（同日，lead 审阅后，两轮）**：修复轮一——写路径去 panic 化（序列化改 `serde_json::to_vec` fallible、互斥锁中毒 `into_inner` 恢复、`Option<File>` 观察点与 `generation_path` 去 expect）、`open_append_regular` 三层校验（非常规条目拒绝、父目录 canonical 前置、打开后复查句柄与 canonical 包含）。修复轮二（superseding）——①重开失败不留死句柄：`rotate_if_large` 入口先重试恢复句柄，重开失败 record 以错误返回不 panic、路径修复后写入自愈（新增符号链接注入测试：rotation 关句柄窗口植入链接 → record `Err` 不写穿，路径修复后自愈）；②会话启动清扫 fail-soft（`RunLog::create` 不因清扫失败阻塞）；③清扫模式收紧为 `shell-<32 hex>.log` / `helper-<32 hex>.stderr.log`（复用 token 校验，`shell-notes.log` 等外来名不动，测试断言）；④12 MiB 阈值表述改为「本轮自选值」，删除「ADR 建议 8–16 MiB」的不实表述（ADR 无数值区间）；⑤README 记录清扫假设同一 user-data 无并发壳（单实例实机多开 NOT RUN）。最终门禁：shell-core `cargo test` **36 项**全绿（26 存量 + logs 单元 9 + log_rotation 集成 3），双 crate Windows 目标 `cargo check` 复跑通过，e2e 假闭环与 ruff 复跑通过。
- **NOT RUN / 边界**：**轮转 Windows 实机行为 NOT RUN**（含对他进程打开文件的 rename 语义、ACL 收紧前的符号链接种植场景实测）；沿袭 Windows 实机 Job Object/WebView2/MSI/NSIS 安装器/签名/自动更新/单实例多开、rfd 对话框实机行为、真实 Redis/RQ 桌面形态、V02-52A N=20、真实供应商。**V02-54C 已经 lead 两轮修复审阅后在 roadmap 勾选；父项 V02-54 保持未勾**，随 Issue #114 继续跟踪；不勾 V02-55 / V02-52B。
