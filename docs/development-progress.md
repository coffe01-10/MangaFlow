...
| H. 主分支可靠性收口 | 进行中 | PR #6 已合并安全与调度修复；P1-8、P2-8 已落地。继续核对其他序号范围、独立性能门禁，并补齐独立队列/数据库和浏览器验收 |
...
以下 P1-7、P1-9～P1-12 的代码修复已由 PR #6 合并，独立集成验收仍待完成；P1-8、P2-8 已在 `agy` 落地，尚未合并。详细复现与原验收标准以 [`roadmap.md`](roadmap.md) 同编号为准。
...
- [x] **P2-8 / 审查 7：并发发布 revision 冲突**。PostgreSQL 行锁 + 唯一键冲突有限重试；耗尽后返回 409，不破坏已发布版本指针。SQLite 已覆盖，未跑 PostgreSQL。
