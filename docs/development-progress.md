...
| I. 安全边界与资源保护 | 已审查，6 项待修复 | P0-3：Next.js 安全升级；P1-13～P1-15：网络与上传入口；P2-4、P2-9：图片异常清理及上游响应限制；局部测试与清理完成不代表安全验收通过 |
...
以下 6 项全部待完成；详细定位、复现、修复要求和验收条件以 [`roadmap.md`](roadmap.md) 同编号为准。P2-4 沿用既有任务并补充本轮证据。

- [ ] **P0-3 / 安全审查 1：Next.js Windows 漏洞升级**。从受影响的 `16.2.10` 升级至官方修复版本 `16.3.3` 或后续已修复版本，核对配置依赖、锁文件并完成 Windows 启动及全量门禁复验。
- [ ] **P1-13 / 安全审查 2：默认本地网络边界**。开发/生产启动显式绑定回环地址，核对所有入口；远程共享另需认证授权，不能依赖 CORS。
- [ ] **P1-14 / 安全审查 3：Docker 数据服务隔离与凭据**。收紧 PostgreSQL/Redis 端口，强化凭据和 Redis 认证/ACL，同步应用、Worker、健康检查与部署说明，不删除用户数据卷。
- [ ] **P1-15 / 安全审查 4：上传请求入口限制**。在 multipart 完整接收前限制实际总量和文件数量，覆盖无长度、额外字段及超限清理，不只在路由检查 20 MB。
- [ ] **P2-4 / 安全审查 5：图片像素限制与失败清理**。捕获解压炸弹等异常并返回 4xx，补像素边界；验证、缩略图或落库失败不留孤儿文件、不误删既有素材。
- [ ] **P2-9 / 安全审查 6：供应商元数据响应限制**。模型发现与余额查询采用有界流式读取，限制解压后内容及模型条目，超限受控失败并释放资源。
