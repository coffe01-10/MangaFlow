...
### 1.1 核心原则
1. **禁止只选最佳结果**：每项测试必须连续运行 N=20 轮，强制保留所有原始运行样本（包括失败与异常轮次），报告 P50 与 P95；P95 使用 nearest-rank `ceil(0.95*N)`，不得更换算法挑选更好结果。
...
正式结果必须绑定不可变环境 manifest：机器型号、CPU/GPU 与驱动、RAM、磁盘、Windows build、浏览器/WebView 版本、Node/Python 版本、电源模式、显示缩放/分辨率、后台进程基线、commit 与构建产物 checksum。缺任一项只可作开发诊断，不进入跨版本门禁。
...
1. **固定独立端口分配**：
   ...
   - 本地 Redis 端口：`6479`
2. **进程树所有权**：
   - 必须复用 `scripts/owned_processes.py` 的 Windows Job Object、canonical run directory、owner token 与 journal；启动时把真实 child 分配进本次 Job。
   - 禁止以裸 PID、PID 枚举或 `taskkill /F /T` 作为清理依据；PID 可重用且不证明资源属于本次运行。
   - 结束时关闭/终止本次 Job Object，并验证端口、journal 与受控目录；任何清理失败向上传播为测试失败。
3. **数据目录隔离与清理**：
...
2. **正式样本**：连续采集 N=20 轮完整数据。加载场景从导航开始到 network-idle + 目标 UI ready；持续交互/FPS 场景固定采样 10 秒；每轮动作脚本、等待条件和采样窗口不得变化。
3. **度量汇总**：
   - 报告：$Min$、$Max$、$P50$（中位数）、$P95$（第 95 百分位值）；
   - 若 20 轮中出现任何 1 轮失败或报错，整项测试判定为未通过，严禁剔除异常值。

### 5.2 核心性能指标与门禁基准（SLA Targets）

| 度量维度 | 指标名称 | 优秀门禁 (Pass) | 警告阈值 (Warn) | 失败阈值 (Fail) |
| --- | --- | --- | --- | --- |
| **首屏与加载** | FCP (首内容绘制) | ≤ 800 ms | 800–1200 ms | > 1200 ms |
| | LCP (最大内容绘制) | ≤ 1500 ms | 1500–2500 ms | > 2500 ms |
| **交互响应** | INP (下次绘制交互延迟) | ≤ 100 ms | 100–200 ms | > 200 ms |
| | 拖拽/画笔输入延迟 | ≤ 16 ms | 16–30 ms | > 30 ms |
| **视觉与帧率** | 滚动与动画平均 FPS | ≥ 55 fps | 45–54 fps | < 45 fps |
| | 掉帧率 (Dropped Frames) | ≤ 3% | 3%–8% | > 8% |
| | 长任务 (Long Task > 50ms) 计数 | ≤ 2 | 3–5 | > 5 |
...
- 机器激活规则：测试清单中的未来 UI 场景在对应 feature manifest 标记 `implemented=true` 且路由/组件探针存在后才激活；此前结果为 `NOT_APPLICABLE`，不能记作 PASS、FAIL 或普通 SKIP。激活提交后缺失探针才判 FAIL。
