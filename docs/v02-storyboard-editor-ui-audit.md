...
- 现有 `bounds.{x,y,width,height}` 解释为页矩形上的归一化轴对齐框。V02-30 落地前，实现 Issue 可以把矩形拖拽映射回该四元组，**前提是** `PATCH /panels/{id}` 接受 `bounds` + `version`。
...
| 格子 | 选中、拖、resize、键盘微调、锁定、阅读序号 | 矩形 bounds 写入：需扩展现有 PATCH；多边形/z：V02-30 |
...
**B. 拖动 / resize 矩形格（V02-30 前仅当 bounds PATCH 已批）**

1. 拖动格体移动；边缘/角 handle resize。
2. Shift 锁定比例；Alt 从中心；吸附开时贴齐相邻边与页边，显示对齐线。
3. 松开写入 `bounds` + `version`。进行中不卸载检查器。
...
2. 松开写该气泡 `region`（及尾巴字段）+ `panel_version`。
3. 跨格拖放 = 移动 `panel_id`。若 V02-30 未定义跨格移动，则拖到格外时回弹并提示「气泡仍属于本格」。
...
- 保存仍按对象：格 PATCH、气泡 PATCH。工具栏「保存本格」保持。
...
定义：1 节点 = 1 个可命中对象（格、气泡、handle 组、对齐线不算节点）。典型压力：约 12 格 × 6 气泡 + 辅助 ≈ 远低于 100；100 节点对应「多气泡试验页」或未来多层标注。
...
检查器字段保存路径保持现有 mutation；画布几何走将新增的 bounds/region 写入。两者共享 `version` / `panel_version`。
...
| --- | --- | --- |
| S1 | 单击格 | 选中轮廓 + 检查器该格 |
| S2 | 拖矩形格松开 | 一次 PATCH bounds+version；刷新后位置仍在 |
...
| S19 | ≥1280px | 画布与检查器并排 |
| S20 | 气泡契约未定时 | 页上无拖拽气泡，检查器 CRUD 仍可用 |
...
本审计 **不** 冻结下列 schema。实现 Issue 若在 V02-30 前开工，只能做视口变换、选中铬、检查器 a11y、离开保护、重建确认，以及——若单独批准——现有矩形 `bounds` 的 PATCH。
...
3. **矩形 bounds 写入（需 V02-30 或单独 API 批准）**：`PanelUpdate.bounds`；拖、resize、吸附、键盘微调、undo。多边形只读。覆盖 S2、S3、S4、S6、S7、S11。
4. **气泡几何（必须 V02-30 region/tail/anchor）**：画布气泡节点、尾巴、可选跨格。覆盖 S20 的后半。
...
- 未打开 V02-30 设计；上节问题在该契约返回前一律 `BLOCKED`。
