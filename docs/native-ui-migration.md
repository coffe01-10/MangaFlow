...
| `/generate` 内嵌导演与局部编辑 | 提议/歧义澄清、命令 diff、确认/拒绝/撤销/重做、选区、局部候选/血缘；`director-workspace.tsx`、`local-edit-workspace.tsx` | 导演台已重写（DirectorPane + DirectorRules 规则桩，envelope 与 web 优先级对齐、支持改口令重发）；局部编辑入口待 NUI-6 对照 | NUI-5 |
...
5. **NUI-5：生成与导演闭环。** 候选/素材库/大图、生成/采用/质检/导出、导演台规则桩
   ...
   实现完成；局部编辑（mask 重绘）入口待 NUI-6 对照。
