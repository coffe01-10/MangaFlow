...
盘点日期：2026-09-08；状态复核：2026-09-21；初始基线：`9e6f738`；当前合并与发布基线：
`master` / `e0b00412`（tag `v1.0.0-rc2`）。以当前源码和 `docs/adr/native-windows-client.md` 为准。旧
...
滚动」从 BLOCKED 推进为有实测（见下文该节）；该轮“暂留 .NET 8”的发布决策随后由
2026-09-20 NUI-10 的独立迁移与复基线验收取代，当前目标为 .NET 10。但“逐页交互
（弹窗/拖放/键盘可达性）枚举”仍有大半是 NOT RUN，NUI-6 不能据这些发布工程格勾选完成。
...
## 2026-09-21 RC2 流程编排展示收口（`91c4111f`，发布 `e0b00412`）

- 流程编排新增 F11 真窗口全屏和 Ctrl+Shift+Enter 专注模式；Esc 依次取消连线、退出
  全屏、退出专注，并恢复此前窗口尺寸与面板状态。专注与全屏可组合。
- “视图”菜单统一节点库、属性、版本与保存状态、运行面板、小地图；选中节点会打开属性，
  同时统一项目内“流程编排”名称和运行工作流入口。
- Shift+滚轮横向移动、Ctrl+滚轮缩放，普通滚轮纵向移动；空格/中键拖动继续平移。
  缩放/节点范围变化后重新计算滚动范围。
- 画布切到共享纸色、墨色、朱红主题，默认 100%，增大节点标题与端口文字；650/940/
  1100/1440 DIP 下改进工具栏换行和侧面板抽屉。
- 原生 Release 工作流和侧栏回归通过，`npm run check` 全绿；证据在
  `output/workflow-ux-20260921/verification.md`。真实多显示器/DPI、长会话帧时间和付费
  Provider/Worker 仍不在该轮结论内。
- `v1.0.0-rc2` Windows x64 NSIS 安装器已发布；全新安装、覆盖升级、40 秒启动探测、
  sidecar 健康握手/停止和卸载通过。安装器未做 Authenticode 签名。
