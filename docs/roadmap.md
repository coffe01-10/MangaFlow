...
- [x] **NUI-1（L2）：原生项目导航和共享页面框架。** 九个项目入口与 Web 顺序/名称一致；
  ...
  原作/任务接入真实视图，其余七页明确显示待迁移清单，尚未实现完整业务 UI。
  断网可即时导航；切页取消旧读取，迟到结果不污染新页，待迁移页不发业务请求。
- [ ] **NUI-2A（L2，下一最高优先级）：完整原生项目设置。** 独立 View 与状态模型，
  对齐 Web 五个分区和模型目录；版本冲突保留输入、重复保存去重、切项目隔离；
  名称确认后软删除并返回首页。不得误用全局运行偏好或修改 API/schema。
- [ ] **NUI-2B（L2）：原作、任务与首页补齐。** 原作修订/删除/撤回/覆盖信息、任务明细
  与完整筛选、首页创建参数逐字段对齐；补齐加载/空/错/脏输入/保存/冲突状态。
- [ ] **NUI-2C（L2）：供应商和全局设置。** 连接/密钥/模型目录/展示偏好/价格/CLI、
  完整运行参数、分层诊断与存储。凭据边界沿用 API，真实验证/付费冒烟单独验收。
- [ ] **NUI-3（L2）：五类资产与漫画剧本。** 子导航与只读列表 → 档案编辑/上传 →
  角色包版本/绑定/风格/场景 → 剧本结构编辑；各边界独立切片，图片按需加载。
- [ ] **NUI-4（L2）：分镜与工作流画布。** 原生画布、工具栏、检查器、缩放/键盘、
  草稿与冲突、运行监视；两类画布分别实现和测量，窄窗口保留检查器访问。
- [ ] **NUI-5（L2）：生成、素材库与导演。** 候选预览/采用/收藏、生产门禁、质检/
  修复/升清/导出、命令确认/撤销、局部编辑/血缘；真实付费闭环单独验收。
...
已验证：原生 Release 构建 0 警告/0 错误，29 项客户端断言和 WPF 九页导航/布局检查
通过，1320/940 DIP 实际 XAML 离屏渲染通过；`npm run check` 通过（Python 1502 passed /
37 skipped，Vitest 434 passed / 42 文件，供应商平权、ESLint、Ruff、TypeScript、Next
生产构建通过；Python 761 条既有弃用警告）。未运行交互式 WPF 会话、浏览器 E2E、
真实供应商、DPI/性能采样或安装验收。本次授权续作仅同步文档，沿用上述已完成验证。
