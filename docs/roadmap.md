...
## 当前优先：网页功能迁移到 WPF 原生客户端（2026-09-07）
...
[原生 UI 迁移清单](native-ui-migration.md)。截至 2026-09-15，当前合并基线为
`master` / `c3f8376`；状态复核开始时工作树干净且与 `origin/master` 同步。下方早期
...
- [ ] **NUI-7（L2/L3 分开）：性能和发布验收。** 多窗口宽度/DPI、启动首帧、导航响应、
  ...
  （draft_version 6→7 + 新边落库；前轮失效系测试脚本坐标系误读）。仍 **NOT RUN**：
  `--render` 收口终跑（首轮遭实机物理鼠标移动干扰，待安静窗口重跑）、多 DPI
  （单屏环境 BLOCKED）、真实代码签名（AUTH-REQ）。NUI-7 父项因此保持未完成。
