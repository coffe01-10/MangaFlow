...
- [ ] **V02-54（L3）：交付 Windows 桌面应用。** 桌面壳必须拥有并验证 API/Worker 子进程生命周期，使用可证明归属的运行目录和 PID/进程树，支持单实例、启动/退出清理、崩溃恢复、日志导出、本地文件选择和安全凭据存储；不得依靠“杀端口”清理未知进程。安装/升级/卸载不得删除用户数据库、素材或凭据。
  ...
  - [ ] **V02-54B（Issue #114 续作，分支 `glm/v02-54b-desktop-ux`，基线 `70639a2`）：** 补第一轮 NOT RUN 中可在 Linux 落地的两块——①**日志导出**：壳侧把统一日志目录（壳 + helper/API/Worker 日志）归档（ZIP）为用户可选路径，归档内容与目标路径均不得穿越用户数据根；②**本地文件选择**：安全文件/目录选择（打开原作/素材等），走壳 API，选择与读取均做规范化路径校验，策略对齐既有上传（PNG/JPEG/WebP ≤ 20 MiB）与原文（TXT/Markdown ≤ 20 MiB UTF-8）边界，不引入任意路径遍历。父项 V02-54 保持未勾；Windows 实机 Job Object/WebView2、安装器、签名、自动更新等仍 NOT RUN。
