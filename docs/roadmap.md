...
- [ ] **V02-54（L3）：交付 Windows 桌面应用。** 桌面壳必须拥有并验证 API/Worker 子进程生命周期，使用可证明归属的运行目录和 PID/进程树，支持单实例、启动/退出清理、崩溃恢复、日志导出、本地文件选择和安全凭据存储；不得依靠“杀端口”清理未知进程。安装/升级/卸载不得删除用户数据库、素材或凭据。
  ...
  - [x] **V02-54C（Issue #117 续作，分支 `glm/v02-54c-log-rotation`，基线 `548b1d1`；经 lead P1+P2 修复轮复核后勾选，父项 V02-54 保持未勾）：** 桌面壳日志轮转——`apps/desktop/shell-core` 对统一 `logs/` 内 `shell-*.log` / `helper-*.stderr.log` 按大小轮转：单文件达阈值（12 MiB，小于导出 64 MiB 上限）后 rename 为带序号世代（`.1`–`.5`），保留有限 5 代、超出删最旧；轮转不跟随符号链接，rename/删除不越 canonical logs 根；壳 RunLog 写前检查、轮转后原打开路径继续可写，helper stderr 按会话 token 分文件、跨会话轮转（取舍在代码与 README 写清）。修复轮收口：RunLog 写路径去 panic 化（序列化 fallible、锁毒 `into_inner` 恢复、重开失败报错不 panic）、`open_append_regular` 打开后复查常规文件与 canonical 包含。门禁：shell-core `cargo test` 35 项、双 crate Windows 目标 `cargo check`、sidecar 假闭环 e2e、ruff。**NOT RUN**：轮转 Windows 实机行为、Windows 实机 Job Object/WebView2/安装器/签名/自动更新、rfd 对话框实机行为、V02-52A N=20、真实供应商。
