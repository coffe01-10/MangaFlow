...
[原生 UI 迁移清单](native-ui-migration.md)。本轮代码基线 `9e6f738`，改动尚未提交或合并。
...
- [ ] **NUI-7（L2/L3 分开）：性能和发布验收。** 多窗口宽度/DPI、启动首帧、导航响应、
  ...
  2026-09-08 状态：**全部 NOT RUN**。本轮未启动真实客户端窗口、未做任何计时采样；
  离屏渲染布局检查（1320/940 等宽度）不构成性能数据。需要的最小实测集：真实启动
  首帧（start-native.ps1 计时）、窗口内页面切换响应、固定数据集长列表内存、双画布
  拖拽帧时间（保留含失败的全部样本）、100/125/150/200% DPI 实机截图、安装/升级与
  退出清理观察。没有上述实测之前，任何格子不得填「完成」。
