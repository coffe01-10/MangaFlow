...
当前开发分支：`glm/v02-54d-windows-status`。V02-53B 桌面壳 PoC 已合并（Issue #110 / PR #111 / `203efad`）；V02-54 第一轮已合并（Issue #114 / PR #115 / `70639a2`，父项 V02-54 保持未勾）；V02-54B 已合并（Issue #114 续作 / PR #116 / `548b1d1`，经 P1 审阅后合入并勾选）；V02-54C 桌面壳日志轮转已合并并勾选（Issue #117 / PR #118 / `efedb08`，同属 Issue #114 跟踪，经 P1+P2 修复轮复核）。当前窗口为 **V02-54D（Issue #119，docs-only）**：Linux 可落地的桌面续作已尽，本轮只汇总仍 NOT RUN / BLOCKED 的 Windows 剩余项状态目录（`docs/v02-windows-leftover-status.md`），不实现。Windows 实机、安装器、签名、自动更新等仍 NOT RUN / BLOCKED；ADR 仍为草案、选型未批准。V02-52B 性能门禁执行仍需 V02-52A N=20 全样本。
