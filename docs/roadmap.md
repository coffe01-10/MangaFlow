...
- [ ] **NUI-7（L2/L3 分开）：性能和发布验收。** 多窗口宽度/DPI、启动首帧、导航响应、
  ...
  用户数据三阶段逐字节零漂移），0.9.0→1.0.0-rc1 跨版本升级路径 PASS。仍 **NOT RUN**：
  G2 长列表内存三档与 600 档（net10 复基线待窗）、`--render` 收口终跑对照、多 DPI
  （单屏环境 BLOCKED）、真实代码签名（AUTH-REQ）。工作流连线拖拽复验同被实机占用
  阻塞（节点拖拽已 PASS）。NUI-7 父项因此保持未完成。
