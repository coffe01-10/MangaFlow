...
当前开发分支：`glm/v02-54c-log-rotation`。V02-53B 桌面壳 PoC 已合并（Issue #110 / PR #111 / `203efad`）；V02-54 第一轮已合并（Issue #114 / PR #115 / `70639a2`，父项 V02-54 保持未勾）；V02-54B 已合并（Issue #114 续作 / PR #116 / `548b1d1`，经 P1 审阅后合入并勾选）。下一实现窗口为 V02-54C（Issue #117，同属 Issue #114 跟踪）：在 `apps/desktop/shell-core` 为统一日志目录 `logs/` 实现**日志轮转**——`shell-*.log` / `helper-*.stderr.log` 按大小轮转（单文件达阈值 rename 为带序号世代，保留有限 N 代，超出删最旧），补 V02-54B 诚实声明的 NOT RUN。Windows 实机、安装器、签名等仍 NOT RUN；ADR 仍为草案、选型未批准。V02-52B 性能门禁执行仍需 V02-52A N=20 全样本。
...
- [ ] **V02-54（L3）：交付 Windows 桌面应用。** 桌面壳必须拥有并验证 API/Worker 子进程生命周期，使用可证明归属的运行目录和 PID/进程树，支持单实例、启动/退出清理、崩溃恢复、日志导出、本地文件选择和安全凭据存储；不得依靠“杀端口”清理未知进程。安装/升级/卸载不得删除用户数据库、素材或凭据。
  ...
  - [ ] **V02-54C（Issue #117 续作，分支 `glm/v02-54c-log-rotation`，基线 `548b1d1`；实现待审阅验收后勾选，父项 V02-54 保持未勾）：** 桌面壳日志轮转——`apps/desktop/shell-core` 对统一 `logs/` 内 `shell-*.log` / `helper-*.stderr.log` 按大小轮转：单文件达阈值（12 MiB，小于导出 64 MiB 上限）后 rename 为带序号世代（`.1`–`.5`），保留有限 5 代、超出删最旧；轮转不跟随符号链接，rename/删除不越 canonical logs 根；壳 RunLog 写前检查、轮转后原打开路径继续可写，helper stderr 按会话 token 分文件、跨会话轮转（取舍在代码与 README 写清）。补 V02-54B 诚实声明的 NOT RUN。Windows 实机项继续 NOT RUN。
