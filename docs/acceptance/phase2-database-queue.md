...
**交付 Commit**: `14332ba499225882ad52fb3066488eb26c728d6b`（第 1 轮返工修复已更新）
...
3. **安全与隔离夹具 (`tests/integration/conftest.py`)**:
   - **URL 白名单与防劫持校验**：强制 PostgreSQL 端口必须为 `55432` 且数据库名必须为 `mangaflow_acceptance`；强制 Redis 端口必须为 `56379` 且 DB 索引必须在 `1..15`（严禁 `DB 0`）；严格拦截 query 参数中的 `host`/`port` 等覆盖劫持。
   - **凭据脱敏**：`mask_url` 对日志与异常文本中的连接 URL 密码进行脱敏（如 `postgresql+psycopg://user:***@127.0.0.1:55432/mangaflow_acceptance`）。
   ...
   - **Redis 命名空间级隔离**：测试 key 均带 `mangaflow:acceptance:{uuid}:*` 专属前缀，清理时仅删除对应前缀 key，严禁无差别 `flushdb`/`flushall`。
4. **真实 PostgreSQL 验收测试 (`tests/integration/test_postgres_acceptance.py`)**:
   ...
   - 验证下游失败时，PostgreSQL 外层事务完整原子回滚与 0 孤立脏数据残留。
...
| **P1-7** | `test_pg_workflow_version_release_concurrency` | PostgreSQL 真实事务行锁竞争，单方成功推进 V2，另一方受控 409 | Implemented | Validated | **BLOCKED**（缺 Docker/PostgreSQL 55432） |
| **P1-10** | `test_pg_transaction_rollback_and_zero_residual_entities` | 下游任务失败时，PostgreSQL 外层事务完整回滚，0 脏批次/候选 | Implemented | Validated | **BLOCKED**（缺 Docker/PostgreSQL 55432） |
| **P1-10** | `test_pg_candidate_creation_blocked_when_batch_closed_during_validation` | 校验期间批次被其他 Session 关闭时返回 409，0 脏数据落库 | Implemented | Validated | **BLOCKED**（缺 Docker/PostgreSQL 55432） |
...
| **SEC** | `test_safe_acceptance_pg_url_blocks_standard_and_remote_and_invalid_dbs` | 拦截 5432、非 acceptance 库、远程 IP 与 query 覆盖 | Implemented | **PASSED** (0.01s) | 无（离线已验证） |
...
| **SEC** | `test_safe_acceptance_redis_url_blocks_standard_ports_and_db_zero` | 拦截 6379、DB 0 (/0, /00, ?db=0) 与远程主机 | Implemented | **PASSED** (0.01s) | 无（离线已验证） |
